...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const moodEntriesResponse = await axios.get('http://127.0.0.1:8000/api/mood-entries/', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        setMoodEntries(moodEntriesResponse.data);
        console.log("Mood entries fetched successfully:", moodEntriesResponse.data);
      } catch (error) {
      ...
      try {
        const journalEntriesResponse = await axios.get('http://127.0.0.1:8000/api/journal-entries/', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        setJournalEntries(journalEntriesResponse.data);
        console.log("Journal entries fetched successfully:", journalEntriesResponse.data);
      } catch (error) {
  ...
  return (
    <div className="container my-5 w-100 p-3 h-100 d-inline-block shadow-lg p-3 mb-5 bg-body-tertiary rounded mt-10 position-absolute top-50 start-50 translate-middle">
      <div className="card p-4 shadow-sm w-100 p-3 h-100 ">
        ...
        <h3 className="text-center mb-4">{profile.user ? profile.user.username : ''}</h3>
